...
type
  TUtilBanco = class
    ...
    public
      ...
      class function AlterarRegistro(const aTabela, aJSON, aColuna, aValor: String; const aIdentificador: Integer): Boolean;
  end;
...
begin
  ...
  FConexao.Params.Add('Database=ZELADORIA_PUBLICA');
...
class function TUtilBanco.AlterarRegistro(const aTabela, aJSON, aColuna, aValor: String; const aIdentificador: Integer): Boolean;
const
  COMANDO_UPDATE = 'UPDATE %s SET %s = "%s" WHERE id = %d';
var
  I: Integer;
  xMemTable: TFDMemTable;
  xQuery: TFDQuery;
...
begin
  ...
  try
    try
      ...
      for I := 0 to Pred(xMemTable.FieldCount) do
      begin
        if xMemTable.Fields[I].AsString = aColuna then
        begin
          if xMemTable.Fields[I].DataType = ftFloat then
          xValor := StringReplace(aValor, ',', '.', [rfReplaceAll]) + ',';
          Result := true;
          break
        end;
      end;
      ...
      xQuery.Connection := FConexao;
      xQuery.SQL.Clear;
      xQuery.SQL.Add(Format(COMANDO_UPDATE, [aTabela, aColuna, xValor, aIdentificador]));
      xQuery.ExecSQL;
      Result := true;
      Self.FecharConexao;
    except
